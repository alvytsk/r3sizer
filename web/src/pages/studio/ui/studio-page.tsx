...
export default function App() {
  ...
  // Image entity (input + params + dirty counter)
  const inputFile = useImageStore((s) => s.inputFile);
  const paramsVersion = useImageStore((s) => s.paramsVersion);
  const setInput = useImageStore((s) => s.setInput);
  const imageError = useImageStore((s) => s.error);
  const resetImage = useImageStore((s) => s.resetImage);
  // Output entity (processed result)
  const outputRgbaData = useOutputStore((s) => s.outputRgbaData);
  const diagnostics = useOutputStore((s) => s.diagnostics);
  const lastProcessedVersion = useOutputStore((s) => s.lastProcessedVersion);
  const clearOutput = useOutputStore((s) => s.clearOutput);
  // Processing feature (lifecycle only — no result state)
  const isProcessing = useProcessingStore((s) => s.isProcessing);
  const progress = useProcessingStore((s) => s.progress);
  const procError = useProcessingStore((s) => s.error);
  const process = useProcessingStore((s) => s.process);
  ...
  const handleOpenFile = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!file) return;
      clearOutput(); // drop stale output from the previous image, cancel any running job
      void setInput(file);
      e.target.value = "";
    },
