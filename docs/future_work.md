...
`metrics::artifact_ratio` and `sharpen::gaussian_blur` are the hot paths.
...
For a fixed sigma, the Gaussian kernel can be computed once and reused across
all probe calls.  Move kernel generation out of the loop.
...
- Diagnostics panel showing all `AutoSharpDiagnostics` fields.
